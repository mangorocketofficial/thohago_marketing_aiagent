...
export const buildInstagramPrompt = (context: InstagramCaptionContext): string => {
  ...
  return [
    ...
    "",
    "[CAROUSEL_PLANNING]",
    "- Default to a 4-slide carousel for most Instagram feed posts.",
    "- Use carousel for educational, step-by-step, before/after, storytelling, list-style, campaign recap, and impact-summary topics.",
    `- Each slide must include role and overlay_texts. Allowed roles: ${slideRoleGuide}.`,
    "- Omit the slides key only when the user explicitly wants a single poster/image or the topic is truly one-shot.",
    "",
    "[REFERENCE_MATERIALS]",
