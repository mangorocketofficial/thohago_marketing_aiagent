...
const markSlotGenerating = async (params: { orgId: string; slot: ScheduleSlotRow }): Promise<ScheduleSlotRow> => {
  ...
  return reloaded.slot_status === "scheduled" ? markSlotGenerating({ orgId: params.orgId, slot: reloaded }) : reloaded;
};

const findCampaignSlot = async (params: { orgId: string; campaignId: string | null }): Promise<ScheduleSlotRow | null> => {
  ...
  if (error) {
    throw new HttpError(500, "db_error", `Failed to load campaign instagram slot: ${error.message}`);
  }
...
const findIdempotentOnDemandSlot = async (params: {
  ...
  if (error) {
    throw new HttpError(500, "db_error", `Failed to query idempotent instagram slot: ${error.message}`);
  }
...
const createOnDemandSlot = async (params: {
  ...
  if (error || !data) {
    throw new HttpError(500, "db_error", `Failed to create instagram on-demand slot: ${error?.message ?? "unknown"}`);
  }
...
/**
 * Resolve target slot with campaign-first then idempotent on-demand fallback.
 */
export const resolveInstagramGenerationSlot = async (params: {
  orgId: string;
  sessionId: string;
...
/**
 * Check if campaign has a pending instagram slot.
 */
export const hasPendingCampaignInstagramSlot = async (params: {
  orgId: string;
  campaignId: string | null;
}): Promise<boolean> => {
  const slot = await findCampaignSlot(params);
  return !!slot;
};
