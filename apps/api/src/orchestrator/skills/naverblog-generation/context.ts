...
export type BlogGenerationContext = {
  ...
  channel: "naver_blog";
  ragContextMeta: {
    context_level: "full" | "partial" | "minimal";
...
const loadCampaignContext = async (params: {
  orgId: string;
  ...
  if (error) {
    console.warn(`[NAVER_BLOG_SKILL] Failed to load campaign context: ${error.message}`);
    return null;
  }
  if (!data) {
    return null;
  ...
  const planText =
    planRaw && typeof planRaw === "object" && !Array.isArray(planRaw)
      ? JSON.stringify(planRaw, null, 2)
      : "";

  const parts = [
    title ? `[Campaign Title]\n${title}` : "",
    planDocument ? `[Campaign Plan Document]\n${planDocument}` : "",
    planText ? `[Campaign Plan JSON]\n${planText}` : ""
  ].filter(Boolean);
  ...
  return truncateToTokenBudget(parts.join("\n\n"), CAMPAIGN_CONTEXT_BUDGET);
};

/**
 * Build the unified RAG context used by naver blog generation.
 */
export const buildBlogGenerationContext = async (params: {
  orgId: string;
  sessionId: string;
  activityFolder: string;
  campaignId: string | null;
  topic: string;
}): Promise<BlogGenerationContext> => {
  ...
  const conversationMemory = truncateToTokenBudget(
    rollingSummary || enriched.memoryMd || "",
    CONVERSATION_MEMORY_BUDGET
  );
